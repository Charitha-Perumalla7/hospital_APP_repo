...
import {
  getServiceRequest, getUsers,
  assignServiceRequest, updateServiceRequestStatus,
...
export default function ServiceRequestDetailPage() {
  ...
  const loadAll = () => {
    getServiceRequest(serviceRequestId)
      ...
      .catch(() => navigate('/service_requests'))

    getComments(serviceRequestId)
      .then(setComments)
      .catch(() => {})

    getAttachments(serviceRequestId)
      .then(setAttachments)
      .catch(() => {})

    getServiceRequestAuditLogs(serviceRequestId)
      .then(setAuditLogs)
      .catch(() => {})
  }
  ...
  const handleAssign = withError(async (e) => {
    ...
    await assignServiceRequest(serviceRequestId, {
  ...
  const handleStatus = withError(async (e) => {
    ...
    await updateServiceRequestStatus(serviceRequestId, {
      status: newStatus,
  ...
  const handleComment = withError(async (e) => {
    ...
    await createComment(serviceRequestId, {
  ...
  const handleDeleteComment = withError(async (commentId) => {
    ...
    await deleteComment(serviceRequestId, commentId)
  })

  const handleAttachment = withError(async (e) => {
    e.preventDefault()

    await createAttachment(serviceRequestId, {
      ...attForm,
      size: parseInt(attForm.size, 10)
    })
  ...
  const handleDeleteAttachment = withError(async (attachmentId) => {
    ...
    await deleteAttachment(serviceRequestId, attachmentId)
  })

  if (!serviceRequest) return <p>Loading…</p>

  const nextStatuses = NEXT_STATUSES[serviceRequest.status] || []

  return (
    <div>
      <button
        className="btn btn-sm btn-outline-secondary mb-3"
        onClick={() => navigate('/service_requests')}
      >
      ...
      <div className="card mb-3">
        <div className="card-body">
          <h5 className="card-title">{serviceRequest.title}</h5>
          ...
          <span
            className={`badge bg-${STATUS_COLORS[serviceRequest.status] || 'secondary'} me-2`}
          >
          ...
          <small className="text-muted">
            ...
            {serviceRequest.assigned_to
              ? (
                users.find(u => u.id === serviceRequest.assigned_to)?.name ||
                serviceRequest.assigned_to
              )
              : <em>unassigned</em>}
          </small>
      ...
      <div className="row g-3">
            ...
            <div className="card-body">
              {nextStatuses.length === 0 ? (
                <p className="text-muted mb-0">
                  Service Request is <strong>{serviceRequest.status}</strong>
          ...
          <div className="card">
            ...
            <div className="card-body">
              ...
              {comments.map(c => (
                <div
                  key={c.id}
                  className="d-flex justify-content-between align-items-start border-bottom py-1"
                >
                  <div>
        ...
        {/* Audit log for this service request */}
        <div className="col-12">
          <div className="card">
            <div className="card-header">
              Audit Log
                  ...
                  <tbody>
                    {auditLogs.map(log => (
                      <tr key={log.id}>
                        ...
                        <td>
                          {users.find(u => u.id === log.performed_by)?.name ||
                            log.performed_by}
                        </td>
